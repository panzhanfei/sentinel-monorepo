...
| `app/api/**`                          | Route Handlers（BFF），如 `auth/refresh` 转发 Node                  |
...
- `app/api/auth/refresh/route.ts` 将刷新请求代理到 Node `POST /auth/refresh`，按 `nodeApiEnvelope` 解析响应体，成功时刷新两个 Cookie 的 `maxAge`（与登录策略一致）。
