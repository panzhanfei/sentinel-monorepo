...
const repoRows = [
  [
    "apps/main-next",
    "Next.js 16（App Router）、RainbowKit / wagmi、Wujie 宿主；源码在 src/，BFF 在 src/app/api。httpOnly 双 JWT（accessToken + refreshToken）；src/proxy.ts 校验受保护路由并对 /api 做子应用 Origin CORS（见 src/wujie/subAppOrigins）。默认端口 3000。",
  ],
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="p-6 rounded-2xl bg-slate-50 border border-slate-200">
                  ...
                  <p className="text-sm text-slate-600 leading-relaxed">
                    BFF 使用{" "}
                    <code className="text-xs bg-white px-1 rounded border">
                      src/lib/bffProxy.ts（上游 JSON 解析等）
                    </code>{" "}
                    等与{" "}
                    <code className="text-xs bg-white px-1 rounded border">
                      {"{ success, data }"}
