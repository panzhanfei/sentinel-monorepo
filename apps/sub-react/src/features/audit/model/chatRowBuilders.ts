...
export function buildStreamErrorRow(
  agent: string,
  message: string,
  opts: IdNowOptions,
): ChatRow {
...
export function buildFirstStreamRow(
  data: { agent?: string; content?: string; status?: string },
  opts: IdNowOptions,
): { row: ChatRow; currentAgent: string } {
...
export function extendLastOrNewStreamRow(
  prev: ChatRow[],
  currentAgent: string,
  data: { agent?: string; content?: string; status?: string },
  opts: IdNowOptions,
): { rows: ChatRow[]; currentAgent: string } {
...
export function buildUserChatRow(message: string, opts: IdNowOptions): ChatRow {
  return {
    id: `local-${opts.newId()}`,
    createdAt: opts.isoNow(),
...
export function buildSystemChatRow(
  message: string,
  opts: IdNowOptions,
): ChatRow {
  return {
